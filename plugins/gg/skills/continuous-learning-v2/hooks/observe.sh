...
CONFIG_DIR="$(_gg_resolve_homunculus_dir)"
...
#   - GG observing its own Haiku observer sessions (self-loop)
#   - GG observing other tools' automated sessions
...
# (GG_HOOK_PROFILE=minimal, GG_SKIP_OBSERVE=1, agent_id, path exclusions).
...
# GG_HOOK_PROFILE is the primary name; ECC_HOOK_PROFILE is a legacy alias.
_CLV2_HOOK_PROFILE="${GG_HOOK_PROFILE:-${ECC_HOOK_PROFILE:-standard}}"
[ "$_CLV2_HOOK_PROFILE" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
# GG_SKIP_OBSERVE is the primary name; ECC_SKIP_OBSERVE is a legacy alias.
_CLV2_SKIP_OBSERVE="${GG_SKIP_OBSERVE:-${ECC_SKIP_OBSERVE:-0}}"
[ "$_CLV2_SKIP_OBSERVE" = "1" ] && exit 0
...
# GG_OBSERVE_SKIP_PATHS is the primary name; ECC_OBSERVE_SKIP_PATHS is a legacy alias.
_CLV2_SKIP_PATHS="${GG_OBSERVE_SKIP_PATHS:-${ECC_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}}"
if [ -n "$STDIN_CWD" ]; then
...
# GG_OBSERVER_SIGNAL_EVERY_N is the primary name; ECC_OBSERVER_SIGNAL_EVERY_N is a legacy alias.
SIGNAL_EVERY_N="${GG_OBSERVER_SIGNAL_EVERY_N:-${ECC_OBSERVER_SIGNAL_EVERY_N:-20}}"
