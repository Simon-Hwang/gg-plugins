...
def _resolve_homunculus_dir() -> Path:
    ...
    if override:
        ...
        print(f"[gg] CLV2_HOMUNCULUS_DIR={override!r} is not absolute; ignoring", file=sys.stderr)

    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        if Path(xdg).is_absolute():
            return Path(xdg) / "gg-homunculus"
        print(f"[gg] XDG_DATA_HOME={xdg!r} is not absolute; ignoring", file=sys.stderr)

    return Path.home() / ".local" / "share" / "gg-homunculus"
