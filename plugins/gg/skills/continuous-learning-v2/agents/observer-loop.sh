...
# GG_OBSERVER_* are the primary names; ECC_OBSERVER_* are legacy aliases.
ANALYSIS_COOLDOWN="${GG_OBSERVER_ANALYSIS_COOLDOWN:-${ECC_OBSERVER_ANALYSIS_COOLDOWN:-60}}"
IDLE_TIMEOUT_SECONDS="${GG_OBSERVER_IDLE_TIMEOUT_SECONDS:-${ECC_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}}"
...
analyze_observations() {
  ...
  # GG_OBSERVER_ALLOW_WINDOWS is the primary name; ECC_OBSERVER_ALLOW_WINDOWS is a legacy alias.
  _CLV2_ALLOW_WINDOWS="${GG_OBSERVER_ALLOW_WINDOWS:-${ECC_OBSERVER_ALLOW_WINDOWS:-false}}"
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "$_CLV2_ALLOW_WINDOWS" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set GG_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  # GG_OBSERVER_MAX_ANALYSIS_LINES is the primary name; ECC_OBSERVER_MAX_ANALYSIS_LINES is a legacy alias.
  MAX_ANALYSIS_LINES="${GG_OBSERVER_MAX_ANALYSIS_LINES:-${ECC_OBSERVER_MAX_ANALYSIS_LINES:-500}}"
  observer_tmp_dir="${PROJECT_DIR}/.observer-tmp"
  mkdir -p "$observer_tmp_dir"
  analysis_file="$(mktemp "${observer_tmp_dir}/gg-observer-analysis.XXXXXX.jsonl")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/gg-observer-prompt.XXXXXX")"
...
- Examples of project patterns: use table-driven Go tests, follow project API conventions
PROMPT
  ...
  # GG_OBSERVER_TIMEOUT_SECONDS / GG_OBSERVER_MAX_TURNS are the primary names; ECC_* are legacy aliases.
  timeout_seconds="${GG_OBSERVER_TIMEOUT_SECONDS:-${ECC_OBSERVER_TIMEOUT_SECONDS:-120}}"
  max_turns="${GG_OBSERVER_MAX_TURNS:-${ECC_OBSERVER_MAX_TURNS:-20}}"
  exit_code=0
  ...
  GG_SKIP_OBSERVE=1 GG_HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print \
