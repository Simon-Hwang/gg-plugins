...
description: Read a plan document, decompose it into steps, design a per-step agent chain from the GG catalogue, and emit ready-to-paste /orchestrate custom prompts. Generative only — never invokes /orchestrate itself. Use when the user has a multi-step plan and wants to drive it through orchestrate without composing chains by hand.
origin: gg
---
...
```
<plan-doc-path> [--lang=python|go|auto] [--scope=all|step:<n>|range:<a>-<b>] [--dry-run]
```
...
## Install form and namespacing

Three install forms determine the prefix on **both** the slash command and every agent name. The two MUST stay in sync — one form per output, never mixed:
...
|---|---|---|---|
| GG plugin install | `<claude-home>/plugins/marketplaces/gg-plugins/` exists | `/gg:orchestrate` | `gg:<name>` |
| ECC plugin install (backward compat) | `<claude-home>/plugins/marketplaces/everything-claude-code/` exists | `/everything-claude-code:orchestrate` | `everything-claude-code:<name>` |
| Legacy bare install | Neither above; agent files under `<claude-home>/agents/` | `/orchestrate` | `<name>` |

Detection runs in the order listed; the first match wins. `gg-plugins` always takes precedence over `everything-claude-code` when both markers exist. If no marker is found, default to legacy and emit: `> Warning: could not detect install form; defaulting to legacy. Edit prefixes if using a plugin install.`

> The ECC plugin detection row exists for backward compatibility with users who have both installs, or who migrated from ECC. New installations use only the GG form.

Why this matters: under plugin install, agents register as `gg:tdd-guide` or `everything-claude-code:tdd-guide`. Bare names force fuzzy matching, which fails intermittently under parallel calls. Under legacy, the prefixed forms are not registered and fail outright.
...
- `refactor-cleaner` — Go/Python dead code, duplicate helper, and import cleanup
...
### Phase 0 — Detect install mode + language

1. Read `<plan-doc-path>`. If missing or empty, report and stop.
2. Detect install form once and freeze it into `INSTALL_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<claude-home>/plugins/marketplaces/gg-plugins/` exists → `INSTALL_MODE=gg-plugin`.
   2. Else if `<claude-home>/plugins/marketplaces/everything-claude-code/` exists → `INSTALL_MODE=ecc-plugin` (backward-compat path for users migrating from ECC).
   3. Else if `<claude-home>/agents/` exists and contains at least one agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `INSTALL_MODE=legacy`.
   4. Else → default to `INSTALL_MODE=legacy` and emit a one-line warning at the top of the output: `> Warning: could not detect install form; defaulting to legacy. If you use a plugin install, edit the prefixes manually.`
   5. If multiple plugin markers exist, `gg-plugins` wins over `everything-claude-code` — the most specific namespace is the only one that resolves agent names without fuzzy matching.
...
3. Resolve `--lang`. When `auto`, run a polyglot-aware detection:
   - Probe markers: `pyproject.toml` / `uv.lock` / `requirements.txt` → python; `go.mod` → go.
...
Emit Markdown using **the form determined by `INSTALL_MODE`**. The output uses one form throughout — every `{ORCH_CMD}` and every agent name is rendered with the matching prefix from Phase 0. **Do not emit both forms; do not include "this is plugin form" / "strip the prefix" instructions in the rendered output.**

Concrete rendering rules:

- `{ORCH_CMD}` = `/gg:orchestrate` under `gg-plugin`, `/everything-claude-code:orchestrate` under `ecc-plugin`, `/orchestrate` under `legacy`.
- `{AGENT(name)}` = `gg:<name>` under `gg-plugin`, `everything-claude-code:<name>` under `ecc-plugin`, `<name>` under `legacy`.
...
**Install mode**: `<gg-plugin | ecc-plugin | legacy>`
**Steps**: <N>
...
- [ ] Every agent in every chain comes from the catalogue (after stripping any plugin prefix that appeared in the plan; see Phase 0 step 5).
...
- **Plan declares agents** (rare): first **strip any plugin prefix** (`gg:`, `everything-claude-code:`) to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `INSTALL_MODE`.
...
### Example 1 — GG plugin mode, Python plan
...
Excerpt of expected output (`INSTALL_MODE=gg-plugin`):
````markdown
...
```bash
/gg:orchestrate custom "gg:tdd-guide,gg:database-reviewer,gg:python-reviewer,gg:security-reviewer" "[Plan: docs/plan/example-feature.md#step-2] Implement EncryptedString SQLAlchemy type and migrate UserProfile.birth_datetime/location columns; key from ENV APP_DB_KEY; Acceptance: encrypt/decrypt roundtrip tests pass; alembic upgrade/downgrade clean on empty DB; no plaintext in DB after migrate; Out of scope: cross-tenant profile sharing logic"
```
````

### Example 2 — Legacy mode, same step

If `INSTALL_MODE=legacy` were detected, the same step would be emitted as a single uniform command (no plugin-prefixed forms anywhere in the output):
