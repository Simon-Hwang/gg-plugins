...
## gg Adaptation Note (UI Microcopy)

This skill is primarily tuned for **long-form writing**: posts, threads, launch notes, essays, blog posts, README framing. It is *not* a substitute for **UX microcopy** discipline (button labels, error messages, empty states, form hints, confirmation dialogs).

When you need to unify UI tone:

1. First derive a brand voice profile with this skill from long-form source material.
2. Then distill it into a short **UX-copy guideline** (one page max), covering at minimum:
   - Verb tense and mood for button labels (imperative vs gerund).
   - Error message shape: `<cause> + <next action>` (no blame, no exclamation marks).
   - Empty-state tone: helpful, not cute.
   - Sentence-end punctuation policy in UI strings.
   - Reserved / forbidden words.
3. Reference the UX-copy guideline directly from your templates and audit PRs, not the long-form voice profile.

The voice profile is the *seed*; the UX-copy guideline is what actually gets enforced on `<button>`, `<label>`, `aria-live` regions, and error toasts.
...
- you need a seed to derive a UX-copy guideline (see Adaptation Note above)
