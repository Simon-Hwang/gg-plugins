...
|---|---|
| Go files, `go.mod`, goroutines, channels, Go API | `golang-patterns`, `golang-testing`, `/gg:tdd`, `/gg:build-fix`, `/gg:review`, `go-reviewer` |
| Python files, `pyproject.toml`, `requirements.txt`, pytest, ruff, mypy | `python-patterns`, `python-testing`, `tdd-workflow`, `/gg:build-fix`, `/gg:review`, `python-reviewer` |
...
4. **Implement in small TDD loops.** Use `tdd-workflow` plus language-specific testing guidance. Use `/gg:tdd` for both Go and Python.
5. **Fix build failures narrowly.** Use `/gg:build-fix` for all stacks (Go, Python, generic). Keep fixes minimal; do not turn build repair into architecture work.
