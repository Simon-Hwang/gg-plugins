...
> Note: `autonomous-loops` is the canonical skill name in GG for loop patterns.
...
> **Note:** NanoClaw (`scripts/claw.js`) is an external script **not included** in gg-plugins. It originates from the [everything-claude-code](https://github.com/disler/everything-claude-code) repository. The pattern is documented here for reference; install it separately if needed.

**NanoClaw** is a session-aware REPL that calls `claude -p` synchronously with full conversation history, persisting sessions to disk as Markdown files.
...
For a self-contained session-persistence alternative, consider writing a short wrapper that reads/appends a `SHARED_TASK_NOTES.md` file across `claude -p` calls (see the Continuous Claude PR Loop pattern below).
...
| NanoClaw | — | `scripts/claw.js` in this repo |
| Verification Loop | — | `skills/verification-loop/` in this repo |
