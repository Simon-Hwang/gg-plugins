...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value GG skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
origin: gg
---
...
This is the GG-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against GG-native coverage
...
- Prefer GG-native workflows over generic "install another plugin" advice when GG can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in GG
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent GG workspaces
...
- existing GG skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but GG lacks a billing-operator skill"
- "Google Drive is connected, but there is no GG-native Google Workspace operator workflow"
...
2. whether GG already has parity
3. whether GG only has primitives
4. whether GG is missing the workflow entirely

### Phase 3: Turn Gaps Into GG Decisions

For every real gap, recommend the correct GG-native shape:

| Gap Type | Preferred GG Shape |
|----------|---------------------|
...
2. **Parity**
   - where GG already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but GG lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete GG-native additions, ordered by impact
...
- If GG already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what GG should own next.
