...
- Use `/gg:build-fix` if build errors occur
- Use `/gg:review` to review completed implementation
- Use the `git-workflow` skill to manage branch, commit, PR description, and release notes

## Optional Planner Agent

GG also provides a `planner` agent for manual installs that include agent files. Use it only when the local runtime already exposes that subagent and the user explicitly asks you to delegate planning.
