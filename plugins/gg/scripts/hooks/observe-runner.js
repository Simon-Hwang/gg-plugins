...
function getPluginRoot(options = {}) {
  ...
  if (process.env.GG_PLUGIN_ROOT && process.env.GG_PLUGIN_ROOT.trim()) {
    return process.env.GG_PLUGIN_ROOT.trim();
  }
...
function getPhaseFromHookId(hookId) {
  const prefix = String(hookId || process.env.GG_HOOK_ID || '').split(':')[0];
  return prefix === 'pre' || prefix === 'post' ? prefix : null;
}

function getTimeoutMs() {
  const parsed = Number.parseInt(process.env.GG_OBSERVE_RUNNER_TIMEOUT_MS || '', 10);
...
if (require.main === module) {
  ...
  const output = run(raw, { hookId: process.argv[2] || process.env.GG_HOOK_ID });
