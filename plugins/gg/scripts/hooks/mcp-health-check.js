...
/**
 * MCP health-check hook for GG plugin.
 *
 * Adapted from everything-claude-code/scripts/hooks/mcp-health-check.js.
 * Compatible with Claude Code hook events:
 ...
 *
 * Health state is persisted outside the conversation context so it survives
 * compaction and later turns.
 *
 * Enable: GG_MCP_HEALTH=on (default)
 * Disable: GG_MCP_HEALTH=off
 * Fail-open: GG_MCP_HEALTH_FAIL_OPEN=1
 */
...
// Preflight HTTP probes check reachability only — no OAuth bearer token.
// Auth-gated responses are treated as reachable; the real MCP client handles auth.
...
function isEnabled() {
  return !['0', 'false', 'off', 'no'].includes(
    String(process.env.GG_MCP_HEALTH || 'on').trim().toLowerCase()
  );
}

function stateFilePath() {
  if (process.env.GG_MCP_HEALTH_STATE_PATH) {
    return path.resolve(process.env.GG_MCP_HEALTH_STATE_PATH);
  }
  return path.join(getClaudeDir(), 'mcp-health-cache.json');
}

function configPaths() {
  if (process.env.GG_MCP_CONFIG_PATH) {
    return process.env.GG_MCP_CONFIG_PATH
...
function markHealthy(state, serverName, now, details = {}) {
  state.servers[serverName] = {
    status: 'healthy',
    checkedAt: now,
    expiresAt: now + envNumber('GG_MCP_HEALTH_TTL_MS', DEFAULT_TTL_MS),
    failureCount: 0,
    lastError: null,
...
function markUnhealthy(state, serverName, now, failureCode, errorMessage) {
  ...
  const backoffBase = envNumber('GG_MCP_HEALTH_BACKOFF_MS', DEFAULT_BACKOFF_MS);
...
function requestHttp(urlString, headers, timeoutMs) {
  return new Promise(resolve => {
    ...
    req.on('error', error => {
      if (settled) return;
      settled = true;
      resolve({ ok: false, statusCode: null, reason: timedOut ? 'request timed out' : error.message });
    });
...
function probeCommandServer(serverName, config) {
  return new Promise(resolve => {
    ...
    const timeoutMs = envNumber('GG_MCP_HEALTH_TIMEOUT_MS', DEFAULT_TIMEOUT_MS);
    const mergedEnv = {
      ...process.env,
    ...
    function attempt(idx) {
      ...
      try {
        child = spawn(tryCommand, args, {
          env: mergedEnv, cwd: process.cwd(), stdio: ['pipe', 'ignore', 'pipe'], shell: useShell
        });
      } catch (error) {
      ...
      timer = setTimeout(() => {
        ...
        attemptFinish({ ok: true, statusCode: null, reason: `${serverName} accepted a new stdio process` });
      }, timeoutMs);
...
async function probeServer(serverName, resolvedConfig) {
  ...
  if (config.type === 'http' || config.url) {
    const result = await requestHttp(
      config.url,
      config.headers || {},
      envNumber('GG_MCP_HEALTH_TIMEOUT_MS', DEFAULT_TIMEOUT_MS)
    );
    return {
      ok: result.ok,
  ...
  return { ok: false, failureCode: null, reason: 'unsupported MCP server config', source: resolvedConfig.source };
}

function reconnectCommand(serverName) {
  const key = `GG_MCP_RECONNECT_${String(serverName).toUpperCase().replace(/[^A-Z0-9]/g, '_')}`;
  const command = process.env[key] || process.env.GG_MCP_RECONNECT_COMMAND || '';
...
function attemptReconnect(serverName) {
  ...
  const result = spawnSync(command, {
    ...
    timeout: envNumber('GG_MCP_RECONNECT_TIMEOUT_MS', DEFAULT_TIMEOUT_MS)
  });
  ...
  if (result.status !== 0) {
    return { attempted: true, success: false, reason: (result.stderr || result.stdout || `reconnect exited ${result.status}`).trim() };
  }
...
function shouldFailOpen() {
  return /^(1|true|yes)$/i.test(String(process.env.GG_MCP_HEALTH_FAIL_OPEN || ''));
}
...
async function handlePreToolUse(rawInput, input, target, statePathValue, now) {
  ...
  if (previous.status === 'unhealthy' && Number(previous.nextRetryAt || 0) > now) {
    logs.push(`[GG:mcp-health] ${target.server} is marked unhealthy until ${new Date(previous.nextRetryAt).toISOString()}; skipping ${target.tool || 'tool'}`);
    return { rawInput, exitCode: shouldFailOpen() ? 0 : 2, logs };
  }

  const resolvedConfig = resolveServerConfig(target.server);
  if (!resolvedConfig) {
    logs.push(`[GG:mcp-health] No MCP config found for ${target.server}; skipping preflight probe`);
    return { rawInput, exitCode: 0, logs };
  }
  ...
  if (probe.ok) {
    ...
    if (previous.status === 'unhealthy') {
      logs.push(`[GG:mcp-health] ${target.server} connection restored`);
    }
    return { rawInput, exitCode: 0, logs };
  }
  ...
  if (probe.failureCode || previous.status === 'unhealthy') {
    ...
    if (reconnect.success) {
      ...
      if (reprobe.ok) {
        markHealthy(state, target.server, now, { source: resolvedConfig.source, restoredBy: 'reconnect-command' });
        saveState(statePathValue, state);
        logs.push(`[GG:mcp-health] ${target.server} connection restored after reconnect`);
        return { rawInput, exitCode: 0, logs };
      }
  ...
  const reconnectSuffix = reconnect.attempted
    ...
    : '';
  logs.push(`[GG:mcp-health] ${target.server} is unavailable (${probe.reason}). Blocking ${target.tool || 'tool'} so Claude can fall back to non-MCP tools.${reconnectSuffix}`);
...
async function handlePostToolUseFailure(rawInput, input, target, statePathValue, now) {
  ...
  logs.push(`[GG:mcp-health] ${target.server} reported ${failureCode}; marking server unhealthy and attempting reconnect`);

  const reconnect = attemptReconnect(target.server);
  if (!reconnect.attempted) {
    logs.push(`[GG:mcp-health] ${target.server} reconnect skipped: ${reconnect.reason}`);
    return { rawInput, exitCode: 0, logs };
  }

  if (!reconnect.success) {
    logs.push(`[GG:mcp-health] ${target.server} reconnect failed: ${reconnect.reason}`);
    return { rawInput, exitCode: 0, logs };
  }

  const resolvedConfig = resolveServerConfig(target.server);
  if (!resolvedConfig) {
    logs.push(`[GG:mcp-health] ${target.server} reconnect completed but no config for follow-up probe`);
    return { rawInput, exitCode: 0, logs };
  }

  const reprobe = await probeServer(target.server, resolvedConfig);
  if (!reprobe.ok) {
    logs.push(`[GG:mcp-health] ${target.server} reconnect command ran, but probe still failed: ${reprobe.reason}`);
    return { rawInput, exitCode: 0, logs };
  }

  const refreshed = loadState(statePathValue);
  markHealthy(refreshed, target.server, now, { source: resolvedConfig.source, restoredBy: 'post-failure-reconnect' });
...
async function main() {
  ...
  if (truncated) {
    const logs = [
      shouldFailOpen()
        ? `[GG:mcp-health] Hook input exceeded ${MAX_STDIN} bytes for ${target.server}; allowing ${target.tool || 'tool'} (fail-open mode)`
        : `[GG:mcp-health] Hook input exceeded ${MAX_STDIN} bytes for ${target.server}; blocking ${target.tool || 'tool'} to protect MCP health gate`
    ];
    emitLogs(logs);
    process.stdout.write(rawInput);
...
main().catch(error => {
  process.stderr.write(`[GG:mcp-health] Unexpected error: ${error.message}\n`);
  process.exit(0);
});
