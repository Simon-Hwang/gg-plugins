 ...
 *
 * Controls:
 * - GG_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - GG_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getHookProfile() {
  const raw = String(process.env.GG_HOOK_PROFILE || 'standard').trim().toLowerCase();
  return VALID_PROFILES.has(raw) ? raw : 'standard';
}

function getDisabledHookIds() {
  const raw = String(process.env.GG_DISABLED_HOOKS || '');
